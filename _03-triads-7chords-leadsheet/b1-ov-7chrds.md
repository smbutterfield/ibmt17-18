...
Because they are simply extended triads, seventh chords are still considered tertian harmony. They are prevalent in almost all styles of Western music, and we have developed many systems to describe how they *function* harmonically. Again though, we must first be able to classify and label them in an objective manner that does not rely on key-based functions. We can do this by analyzing their intervallic structure. We name the chord members by the distance above the bottom pitch **when the chord is stacked in thirds**:
